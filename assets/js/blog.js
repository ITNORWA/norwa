(() => {
    ...
    if (videoGridEl) {
      if (!featuredVideos.length) {
        ...
        featuredVideos.forEach((post) => {
          ...
          const img = post.coverImage || "assets/img/blog/blog-1.jpg";
  ...
  const load = async () => {
    ...
    // Added mainImage to query to get valid URLs
    ...
    try {
      ...
      listEl.innerHTML = '<div style="text-align:center; width:100%; padding: 50px;"><div class="spinner-border text-success" role="status"></div></div>';
      ...
      // If using AOS (Animate On Scroll), refresh it after loading
      if(window.AOS) window.AOS.refresh();
      
    } catch (err) {
      console.error(err);
      listEl.innerHTML = "<p style='text-align:center; width:100%;'>Unable to load posts.</p>";
    }
  };
